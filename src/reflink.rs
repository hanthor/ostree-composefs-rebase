...
pub fn reflink<P: AsRef<Path>, Q: AsRef<Path>>(src: P, dest: Q) -> Result<()> {
    let src_file = File::open(&src)
        .with_context(|| format!("failed to open source file for reflink: {}", src.as_ref().display()))?;
    
    let dest_file = File::create(&dest)
        .with_context(|| format!("failed to create destination file for reflink: {}", dest.as_ref().display()))?;
    ...
    fn test_reflink() {
        ...
        match reflink(&src_path, &dest_path) {
            ...
            Err(e) => {
                ...
                println!("Reflink failed (expected on unsupported filesystems): {:?}", e);
            }
        }
    }
}
